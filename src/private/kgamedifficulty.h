...
public:
     ...
     *
     * Does the game have to be restarted, when the player changes the difficulty level? In this case and if the game is running, a confirmation dialog is
     * displayed and the player can cancel the change.
     */
    enum onChange {
    ...
    enum standardLevel {
        ...
        Configurable = 90, /**< Level "Custom". This is a special item to let the player configure the difficulty level. The configuration of the user level has
                              to be implemented in each game using it with an adapted dialog. Example: In a minesweeper game like KMines, the player wants to
     ...
     * @param slotStandard Slot to call when the player changed the difficulty level to a standard one. Slot should have the signature like:
     ...
     * @param key Custom level identifier. (It must be distinct for every different level. Trying to add a new level with an allocated key replace the previous
     * level.).
