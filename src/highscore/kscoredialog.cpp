...
#include <kdegameswidgets_highscore_logging.h>
// KF
#include <KConfig>
#include <KLineEdit>
...
void KScoreDialog::addLocalizedConfigGroupName(const QPair<QByteArray, QString> &group)
{
    if (!d->translatedGroupNames.contains(group.first)) {
        ...
        qCDebug(KDEGAMESWIDGETS_HIGHSCORE_LOG) << "adding" << group.first << "->" << group.second;
    }
}
...
void KScoreDialogPrivate::aboutToShow()
{
    ...
    for (; it != scores.constEnd(); ++it) {
        ...
        if (hiddenGroups.contains(groupKey))
            continue;
        qCDebug(KDEGAMESWIDGETS_HIGHSCORE_LOG) << latest.first << tabWidget->tabText(tabIndex);
        ...
        for (int i = 1; i <= 10; ++i) {
            ...
            // qCDebug(KDEGAMESWIDGETS_HIGHSCORE_LOG) << "groupName:" << groupName << "id:" << i-1;
...
void KScoreDialogPrivate::loadScores()
{
    ...
    {
        qCDebug(KDEGAMESWIDGETS_HIGHSCORE_LOG) << "The current high score group " << configGroup << " isn't in the list, adding it";
    ...
    for (const QByteArray &groupKey : groupKeys) {
        if ((scores[groupKey][0].value(KScoreDialog::Score) == QLatin1String("-")) && (scores.size() > 1) && (latest.first != groupKey)) {
            qCDebug(KDEGAMESWIDGETS_HIGHSCORE_LOG) << "Removing group " << groupKey << " since it's unused.";
...
{
    qCDebug(KDEGAMESWIDGETS_HIGHSCORE_LOG) << "adding new score";
    ...
    for (int i = 0; i < d->scores[d->configGroup].size(); i++) {
        ...
        qCDebug(KDEGAMESWIDGETS_HIGHSCORE_LOG) << "num_score =" << num_score << " - newScore =" << newScore;
