...
public:
    ...
    void setTextColor(const KStatefulBrush &brush)
    {
        m_brush = brush;
    }
...
void KGamePopupItem::paint(QPainter *p, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
    ...
    p->drawPath(d->m_path);
    p->drawPixmap(MARGIN, static_cast<int>(d->m_boundRect.height() / 2) - d->m_iconPix.height() / 2.0 / d->m_iconPix.devicePixelRatio(), d->m_iconPix);
    p->restore();
}
...
void KGamePopupItem::showMessage(const QString &text, Position pos, ReplaceMode mode)
{
    ...
    d->m_boundRect.adjust(-borderRadius, -borderRadius, borderRadius, borderRadius);
    ...
    d->m_textChildItem->setPos(d->m_textChildItem->x(), d->m_boundRect.height() / 2 - d->m_textChildItem->boundingRect().height() / 2);
...
void KGamePopupItem::setupTimeline()
{
    ...
    if (d->m_position == TopLeft || d->m_position == TopRight) {
        ...
        setPos(d->m_visibleSceneRect.left() + d->m_visibleSceneRect.width() / 2 - d->m_mappedBoundRect.width() / 2,
               d->m_visibleSceneRect.top() + d->m_visibleSceneRect.height() / 2 - d->m_mappedBoundRect.height() / 2);
    }
}
